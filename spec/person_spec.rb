...
describe Person do
    ...
    describe 'can manage funds if account been created' do
        ...
        it 'can withdraw funds' do
            command = lambda { subject.withdraw(amount: 100, pin: subject.account.pincode, account: subject.account, atm: atm) }
            expect(command.call).to be_truthy
        end

        it 'withdraw is expected to raise an error if no ATM is passed in' do
            command = lambda { subject.withdraw(amount: 100, pin: subject.account.pincode, account: subject.account) }
        ...
        it 'funds are added to cash - deducted from account balance' do
            subject.cash = 100
            subject.deposit(100)
            subject.withdraw(amount: 100, pin: subject.account.pincode, account: subject.account, atm: atm)
