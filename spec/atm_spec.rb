...
describe Atm do
  ...
  it 'rejects withdraw if the ATM has insufficient funds' do
    subject.funds = 50
    expected_output = { status: false, message: 'insufficient funds in ATM', date: Date.today }
    expect(subject.withdraw(100, '1234', account)).to eq expected_output
  end

  it 'ATM rejects withdraw if the pin is wrong' do
    expected_output = { status: false, message: 'pin is wrong', date: Date.today }
