...
describe Atm do
    let(:account) { instance_double('Account', pincode: '1234', exp_date: '04/22', account_status: :active) }
    ...
    it 'reject withdraw if account status is disabled' do
        allow(account).to receive(:account_status).and_return(:disabled)
        expected_output = { status: false, message: 'account status is disabled', date: Date.today }
        expect(subject.withdraw(100, '1234', account)).to eq expected_output
    end
end
