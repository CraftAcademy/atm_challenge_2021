...
describe Atm do
  let(:account) { instance_double('Account', pin_code: '1234', exp_date: '04/26', account_status: :active) }
  ...
  it 'is expected to allow withdrawal if the account has enough balance' do
    expected_output = { 
      status: true, 
      message: 'success', 
      date: Date.today, 
  ...
  it 'is expected to reject withdraw if account has insufficient funds' do
  ...
  it 'is expected to reduce funds at withdraw' do
